...
//        log.debug("sign string : " + sb.toString());
        // 将字符串md5后返回（小写）
    ...
    public static void main(String[] args) {
        ...
        param.put("mobile", "18611083559");
        param.put("random", "2329830201238");

        String token = SignUtil.createToken(param, "abcdefghijklmnopqrsuvwxyz123456", 0);

        System.out.println(token);

        Map<String, String> re1 = SignUtil.tokenClaim(token);

        System.out.println(JsonUtil.object2Json(re1));

        Map<String, String> re = SignUtil.verifyToken(token, "abcdefghijklmnopqrsuvwxyz123456");

        System.out.println(JsonUtil.object2Json(re));
    }
